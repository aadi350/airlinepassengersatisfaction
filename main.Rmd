...
From the following LRT using only the most significant parameters above. From the results below, we can see that the the p-value is near zero with the $\chi^{2}$ statistic shows that the $\beta s$ for the attributes omitted are statistically highly likely to be non-zero
```{r}
library(lmtest)
...
print(r_squared)
```
Comparing fit to the reduced model above, the $R^{2}$ value was lower than that of the above,
```{r}
require(rms)
...
```
Given the above prediction, we can re-check the original dataframe, given that the above prediction was less than zero, the logistic regression model would have correctly classified the tuple into the 'not satisified/neutral' category 
```{r}
